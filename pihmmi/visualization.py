...
def find_optimal_threshold_newick(newick_str, threshold_range=(0.1, 1.0, 0.05)):
    """
    Find optimal clustering threshold based on silhouette score for a phylogenetic tree in Newick format.
    ...
    threshold_range : tuple
        Tuple of (min_threshold, max_threshold, step) to test thresholds.
    ...
    for t in thresholds:
        cluster_ids = fcluster(linkage_matrix, t, criterion='distance')
        if len(np.unique(cluster_ids)) > 1:  # Need at least 2 clusters
            score = silhouette_score(distances, cluster_ids, metric='precomputed')
            if score > best_score:
                best_score = score
                best_threshold = t
